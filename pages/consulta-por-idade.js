...
export default function Consulta() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const res = await axios.get('/api/consultar-idade', {
                headers: {
    ...
    return (
        <div className="flex flex-col px-16 py-4 w-full">
            <h1 className="mx-auto text-5xl font-poppins">Outros tipos de consulta</h1>
            <p className="mx-auto mt-8">SELECT * FROM alunos WHERE idade = 13;</p>
            <p className="mx-auto">SELECIONE todos DE alunos ONDE idade = 13;</p>
            ...
            {alunos.length > 0 && (
                <div className="flex flex-col">
                    ...
                    <Link href='/jogo-da-discordia' className='mx-auto mt-8 p-4 text-xl text-blue-800'>Próximo &gt;&gt;</Link>
                </div>
            )}
        </div>
    )
