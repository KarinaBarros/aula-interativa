...
export default function ResultadoEngracado() {
    ...
    useEffect(() => {
        ...
        if (tokenStorage) {
            const fetchData = async () => {
                try {
                    const res = await axios.get('/api/resultado-engracado', {
                        headers: {
    ...
    return(
        <div className="flex flex-col px-16 py-4 h-screen w-full">
            ...
            <h2 className="mx-auto text-xl">Mais engraçado</h2>
            {alunos.length>0 ? (
                <div className="mx-auto mt-8">
                    ...
                    <Link href='/teste' className='mx-auto p-4 text-xl text-blue-800'>Próximo &gt;&gt;</Link>
                </div>
            ) : 
                <LottieAnimation/>
            }
